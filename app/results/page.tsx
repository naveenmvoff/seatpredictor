...
export default function Results() {
  ...
  const [predictorData, setPredictorData] = useState<PredictorData | null>(null)
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      const filtered = colleges.filter((college) => {
        ...
        const matchesSpecialization = !data.specialization || college.specialization === data.specialization
        const matchesCourse = college.course === data.course
        const matchesCategory = college.category === data.category

        return matchesRank && matchesState && matchesSpecialization && matchesCourse && matchesCategory
      })
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        <div className="hidden sm:block">
          <Breadcrumb
            items={[{ label: "Home", href: "/" }, { label: "Seat Predictor", href: "/" }, { label: "NEET PG" }]}
          />
        </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-8 text-gray-600">
              <div className="flex items-center">
                ...
                <span className="text-sm">Know which institute is waiting for you</span>
              </div>
              ...
              <div className="flex items-center">
                ...
                <span className="text-sm">Collated as per previous year seat allotment data</span>
              </div>
            </div>

            <div className="hidden lg:flex items-center bg-white border border-gray-200 rounded-lg px-4 py-2">
              <span className="text-sm font-medium text-gray-700 mr-2">Personal details</span>
              <div className="text-xs text-gray-500 flex items-center">
                <span>{predictorData.name || "Rajesh Das"}</span>
                <span className="mx-1">•</span>
                <span>+91 XXXXXXX</span>
                <span className="mx-1">•</span>
                <span>{predictorData.email || "rajesh@example.com"}</span>
                <span className="mx-1">•</span>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">NEET PG 2024 Rank</label>
              <input
                type="number"
                value={predictorData.rank}
                readOnly
            ...
            <div>
              ...
              <select
                value={predictorData.state}
                disabled
                className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-sm"
              >
                <option value={predictorData.state}>{predictorData.state}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select
                value={predictorData.course}
                disabled
                className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-sm"
              >
                <option value={predictorData.course}>{predictorData.course}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select
                value={predictorData.specialization}
                disabled
                className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-sm"
              >
                <option value={predictorData.specialization}>{predictorData.specialization}</option>
              </select>
            </div>
          </div>

          <div className="flex justify-center sm:justify-end mt-4">
            <button className="w-full sm:w-auto bg-slate-700 hover:bg-slate-800 text-white px-6 py-2 rounded-md font-medium transition-colors">
              Update
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
          ...
          <div className="block sm:hidden">
            <div className="p-4 border-b bg-gray-50">
              <h3 className="font-medium text-gray-900">Search Results ({filteredColleges.length} colleges found)</h3>
            </div>
              ...
              {currentColleges.map((college, index) => (
                <div key={college.id} className="p-4">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="text-sm font-medium text-gray-900">{college.rank.toLocaleString()}</div>
                  </div>
                  ...
                  <div className="mt-2 text-sm font-medium text-gray-900">{college.state}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {currentColleges.map((college, index) => (
                  <tr key={college.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{college.state}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div className="flex items-center space-x-2">
                <span className="text-sm text-gray-700">
                  {startIndex + 1}-{Math.min(endIndex, filteredColleges.length)} of {filteredColleges.length}
                </span>
                <select
                  value={rowsPerPage}
                  onChange={(e) => {
